...
class nnUNetLogger(object):
    ...
    def __init__(self, verbose: bool = False):
        self.my_fantastic_logging = {
            ...
            'val_classification_acc': list(),
            'val_classification_f1': list(),
            'val_dice_per_class': list(),
            'val_mean_fg_dice': list(),
            'val_dice_lesion_only':list(),
    ...
    def log(self, key, value, epoch: int):
        ...
        # wandb logging hook
        self.log_wandb(key, value, epoch)
        
    def log_wandb(self, key, value, epoch):
        """Log selected metrics to wandb if active and allowed"""
        if wandb.run is None:
            return
    
        if key in [
            'train_losses', 'val_losses',
            'val_classification_acc', 'val_classification_f1',
            'val_dice_per_class', 'val_mean_fg_dice',
            'val_dice_whole_pancreas', 'val_dice_lesion_only'
        ]:
            try:
                wandb.log({key: value, "epoch": epoch})
            except Exception as e:
                print(f"[wandb] Failed to log {key}: {e}")
    ...
    def plot_progress_png(self, output_folder):
        ...
        # Plot Dice and F1 on secondary y-axis
        ax2.plot(x_values, self.my_fantastic_logging['val_mean_fg_dice'][:epoch + 1],
                 color='g', ls='dotted', label="val mean FG Dice", linewidth=3)
        
        ax2.plot(x_values, self.my_fantastic_logging['val_classification_f1'][:epoch + 1],
                 color='m', ls='-', label="val classification F1", linewidth=4)
