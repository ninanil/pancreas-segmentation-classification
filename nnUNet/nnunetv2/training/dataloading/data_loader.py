...
class nnUNetDataLoader(DataLoader):
    ...
    def generate_train_batch(self):
        ...
        # Preallocate memory for segmentation data
        data_all = np.zeros(self.data_shape, dtype=np.float32)
        seg_all = np.zeros(self.seg_shape, dtype=np.int16)
        
        # hold classification labels
        class_labels = []

        for j, i in enumerate(selected_keys):
            # Determine whether this patch should contain foreground
            ...
            # Crop and pad image and segmentation
            data_all[j] = crop_and_pad_nd(data, bbox, pad_value=0)

            seg_cropped = crop_and_pad_nd(seg, bbox, pad_value=-1)
            if seg_prev is not None:
                seg_cropped = np.vstack((seg_cropped, crop_and_pad_nd(seg_prev, bbox, -1)[None]))
            seg_all[j] = seg_cropped

            # Fetch classification label
            if hasattr(self._data, "class_label_map"):
                case_id = i
                class_labels.append(self._data.class_label_map[case_id])

        # If 2D mode, remove z-dimension
        if self.patch_size_was_2d:
            data_all = data_all[:, :, 0]
            seg_all = seg_all[:, :, 0]
        ...
        if self.transforms is not None:
            ...
            # Return batch including class labels
            return {
                'data': data_all,
                'target': seg_all,
                'keys': selected_keys,
                'class': torch.tensor(class_labels).long()
            }

        #  Also return class labels if no transforms are applied
        return {
            'data': data_all,
            'target': seg_all,
            'keys': selected_keys,
...
if __name__ == '__main__':
    ...
    # Use the correct plans name if you're using ResEnc M
    dataset_name = 'Dataset050_MyProject'

    folder = join(nnUNet_preprocessed, 'Dataset050_MyProject', 'nnUNetPlans_3d_fullres')

    dataset_json = load_json(join(nnUNet_preprocessed, dataset_name, 'dataset.json'))

    ds = nnUNetDatasetWithClassification(folder)
    pm = PlansManager(join(nnUNet_preprocessed, dataset_name, 'nnUNetResEncUNetMPlans.json'))
    lm = pm.get_label_manager(dataset_json)

    # Setup DataLoader (example with batch size 2 and patch size 64³)
    dl = nnUNetDataLoader(
        ds,
        batch_size=2,
        patch_size=(64, 64, 64),
        final_patch_size=(64, 64, 64),
        label_manager=lm,
